...
import com.jungma.book.springboot.domain.user.Role;
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(authorize -> {
                    ...
                    // "/api/v1/**" 주소를 가진 API 는 USER 권한을 가진 사람만 가능하도록 한다.
                    authorize.requestMatchers("/", "/css/**", "/images/**",
                                    "/js/**", "/h2-console/**").permitAll()
                            .requestMatchers("/api/v1/**").hasRole(Role.USER.name())
                            // 설정된 값들 이외 나머지 URL 들을 나타낸다.
                            // authenticated() 를 추가하여 나머지 URL 들은 모두 인증된 사용자들에게만 허용한다.
